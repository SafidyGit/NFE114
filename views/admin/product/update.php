  ...
  <form method="POST" action="/index.php?action=product_update&id=<?= $id ?>">
    ...
    <div class="row">
      ...
      <div class="col-md-6 mb-3">
        ...
        <input type="number" step="0.01" class="form-control bg-secondary text-white border-0 shadow-none" name="product_quantity_stock" value="<?= $product['product_quantity_stock']?>" required>
      </div>

      <div class="col-md-6 mb-3">
        <label for="product_alert_threshold" class="form-label">Seuil d'alerte</label>
        <input type="number" step="0.01" class="form-control bg-secondary text-white border-0 shadow-none" name="product_alert_threshold" value="<?= $product['product_alert_threshold']?>" required>
      </div>

      <div class="col-md-6 mb-3">
        <label for="product_unit_price" class="form-label">Prix unitaire (€)</label>
        <input type="number" step="0.01" class="form-control bg-secondary text-white border-0 shadow-none" name="product_unit_price" value="<?= $product['product_unit_price']?>" required>
      </div>
